...
export default function Header() {
  ...
  return (
    <header className={`fixed w-full z-50 transition-transform duration-300 ${hidden ? '-translate-y-full' : 'translate-y-0'}`}>
      <div className="bg-white/95 backdrop-blur border-b">
        <div className="container mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8 text-sm">
            <a className="relative pb-1 hover:text-dsBlue transition" href="#services">
              <span>Services</span>
              {/* example hover/active indicator (underline on hover) */}
              <span className="absolute left-0 -bottom-1 w-0 h-[2px] bg-dsPink transition-all group-hover:w-full" />
            </a>
            <a href="#work" className="hover:text-dsBlue">Work</a>
            <a href="#about" className="hover:text-dsBlue">About</a>
            <a href="#blog" className="hover:text-dsBlue">Blog</a>
          ...
          <div className="md:hidden">
            ...
            >
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden>
                <path d="M4 6h16M4 12h16M4 18h16" stroke="#0F2B3B" strokeWidth="1.5" strokeLinecap="round"/>
              </svg>
            </button>
          </div>
        </div>
